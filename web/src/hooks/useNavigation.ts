...
import { DEFAULT_CELL_W, DEFAULT_CELL_H, ROW_HEADER_W, SCROLLBAR_SIZE, scrollBounds, headerHeightForZoom } from "../lib/viewport";
...
export function useNavigation() {
  ...
  const navigateTo = (scrollX: number, scrollY: number, pushHistory = false) => {
    ...
    const { zoom, minimapVisible, minimapWidth, pushNavHistory } = useUiStore.getState();
    ...
    const gridH = Math.max(0, viewport.height - headerH - SCROLLBAR_SIZE);
