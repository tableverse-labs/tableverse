...
impl BatchAcc {
    ...
    fn update_moments(&mut self, v: f64) {
        let n_1 = self.moment_n as f64;
        let n = n_1 + 1.0;
        let delta = v - self.moment_mean;
        let delta_n = delta / n;
        let delta_n2 = delta_n * delta_n;
        let term1 = delta * delta_n * n_1;
        self.m4 += term1 * delta_n2 * (n_1 * n_1 - n_1 + 1.0) + 6.0 * delta_n2 * self.m2
            - 4.0 * delta_n * self.m3;
        self.m3 += term1 * delta_n * (n_1 - 1.0) - 3.0 * delta_n * self.m2;
        self.m2 += term1;
        self.moment_mean += delta_n;
        self.moment_n += 1;
    }

    fn skewness(&self) -> Option<f64> {
        if self.moment_n < 3 || self.m2 == 0.0 {
            return None;
        }
        let n = self.moment_n as f64;
        Some(n.sqrt() * self.m3 / self.m2.powf(1.5))
    }

    fn kurtosis(&self) -> Option<f64> {
        if self.moment_n < 4 || self.m2 == 0.0 {
            return None;
        }
        let n = self.moment_n as f64;
    ...
    fn merge(&mut self, other: BatchAcc) {
        ...
        let n_a = self.moment_n as f64;
        let n_b = other.moment_n as f64;
        let n = n_a + n_b;
        if n > 0.0 {
            let delta = other.moment_mean - self.moment_mean;
            ...
            self.m2 += other.m2 + delta2 * n_a * n_b / n;
            self.moment_mean = (n_a * self.moment_mean + n_b * other.moment_mean) / n;
            self.moment_n += other.moment_n;
        }

        self.zero_count += other.zero_count;
        self.infinite_count += other.infinite_count;
    }
}
...
pub fn compute_column_stats(
    ...
    let completeness_score = 1.0 - null_rate;
    let class_imbalance_ratio = top_values.as_ref().and_then(|tv| {
        if tv.len() >= 2 {
            let max_count = tv.first().map(|v| v.count).unwrap_or(0);
            let min_count = tv.last().map(|v| v.count).unwrap_or(0);
            if min_count > 0 {
                Some(max_count as f64 / min_count as f64)
            } else {
                None
            }
        } else {
...
fn compute_column_stats_parquet_streaming(
    ...
    let out = compute_streaming_stats(
        path, col_idx, is_numeric, meta_min, meta_max, total_rows, n_bins,
    )?;

    let cardinality_category = classify_cardinality(out.distinct_count, total_rows);
    let completeness_score = 1.0 - null_rate;

    let outlier_pct = if is_numeric {
        match (&out.quantiles, &out.histogram) {
            (Some(q), Some(hist)) => {
                let non_null = total_rows - null_count;
                if non_null > 0 {
                    let outlier_count: u64 = hist
                        .iter()
                        .filter(|b| b.hi <= q.p1 || b.lo >= q.p99)
                        .map(|b| b.count)
                        .sum();
                    Some(outlier_count as f64 / non_null as f64)
                } else {
                    None
                }
            }
    ...
    let class_imbalance_ratio = out.top_values.as_ref().and_then(|tv| {
        if tv.len() >= 2 {
            let max_count = tv.first().map(|v| v.count).unwrap_or(0);
            let min_count = tv.last().map(|v| v.count).unwrap_or(0);
            if min_count > 0 {
    ...
    Ok(ColumnStats {
        ...
        null_count,
        null_rate,
        distinct_count: Some(out.distinct_count),
        min,
        max,
        mean: out.mean,
        quantiles: out.quantiles,
        histogram: out.histogram,
        top_values: out.top_values,
        cardinality_category,
        skewness: out.skewness,
        kurtosis: out.kurtosis,
        zero_count: out.zero_count,
        infinite_count: out.infinite_count,
        outlier_pct,
        completeness_score,
        class_imbalance_ratio,
    })
...
fn compute_streaming_stats(
    ...
    total_rows: u64,
    n_bins: usize,
) -> Result<StreamingStatsOutput, EngineError> {
    ...
    let skewness = if is_numeric { merged.skewness() } else { None };
    let kurtosis = if is_numeric { merged.kurtosis() } else { None };
    let zero_count = if is_numeric {
        Some(merged.zero_count)
    } else {
        None
    };
    let infinite_count = if is_numeric {
        Some(merged.infinite_count)
    } else {
        None
    };
